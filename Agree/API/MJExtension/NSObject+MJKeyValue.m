...
- (void)setKeyValues:(NSDictionary *)keyValues
{
    ...
    [self enumerateIvarsWithBlock:^(MJIvar *ivar, BOOL *stop) {
        ...
        if ([value isKindOfClass:[NSNull class]]) {
            ...
            //如果是NSNumber类型
            if ([ivar.type.code isEqualToString:@"NSNumber"]) {
                NSNumberFormatter *numFormat = [[NSNumberFormatter alloc] init];
                numFormat.numberStyle = NSNumberFormatterDecimalStyle;
                if (![value isKindOfClass:[NSNumber class]]) {
                    value = [numFormat numberFromString:value];
                } 
            } else if ([ivar.type.code isEqualToString:@"NSDate"]) {
                //如果是NSDate类型
                NSDateFormatter *fmt = [[NSDateFormatter alloc] init];
                fmt.locale = [[NSLocale alloc] initWithLocaleIdentifier:@"zh_CN"];
                fmt.dateFormat = @"yyyy-MM-dd HH:mm:ss";
                value = [fmt dateFromString:value];
            }else {
...
- (NSDictionary *)keyValues
{
    ...
    [self enumerateIvarsWithBlock:^(MJIvar *ivar, BOOL *stop) {
        ...
        if ([value isKindOfClass:[NSDate class]]) {
            keyValues[key] = [SRTool dateToString:value];
        }else {
            keyValues[key] = value;
        }
    }];
