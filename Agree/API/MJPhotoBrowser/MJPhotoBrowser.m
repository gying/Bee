...
{
    _nowPhotoView = [self dequeueReusablePhotoView];
    if (!_nowPhotoView) { // 添加新的图片view
        _nowPhotoView = [[MJPhotoView alloc] init];
        _nowPhotoView.photoViewDelegate = self;
    }
    ...
    _nowPhotoView.tag = kPhotoViewTagOffset + index;
    
    MJPhoto *photo = _photos[index];
    _nowPhotoView.frame = photoViewFrame;
    _nowPhotoView.photo = photo;
    
    [_visiblePhotoViews addObject:_nowPhotoView];
    [_photoScrollView addSubview:_nowPhotoView];
